...
1. **Entry**: FastAPI endpoint receives the PDF as a binary stream in RAM (no disk storage needed).
2. **Process**: The "Brain" extracts header data and table items, cleaning strings into integers/floats.
3. **Storage**: The "Memory" (SQLAlchemy) performs an **Upsert**—checking if clients/products exist before creating new records to avoid duplication.
